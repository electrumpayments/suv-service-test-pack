...
public class RedeemVoucherHandler extends BaseHandler {
   ...
   public Response handle(RedemptionRequest redemptionRequest, UriInfo uriInfo) {
      try {
         ...
         RequestKey key = addRedemptionRequestToCache(voucherCode, redemptionRequest);
         // TODO See Giftcard, should this all be done differently
         RedemptionResponse redemptionRsp = VoucherModelUtils.redemptionRspFromReq(redemptionRequest); //TODO could change this to user voucherCode...?
   ...
   private void addRedemptionResponseToCache(RequestKey key, RedemptionResponse redemptionRsp) {
       ConcurrentHashMap<RequestKey, RedemptionResponse> responseRecords = SUVTestServerRunner.getTestServer().getRedemptionResponseRecords();
